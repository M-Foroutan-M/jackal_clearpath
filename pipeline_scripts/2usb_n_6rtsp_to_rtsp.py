...
def load_config():
    ...
    settings_path = script_dir / "settings" / "2usb_n_6rtsp_to_rtsp_SETTINGS.json"
    default_path = script_dir / "settings" / "default" / "2usb_n_6rtsp_to_rtsp_SETTINGS_DEFAULT.json"
...
def validate_config(config):
    ...
    if not isinstance(labels, list) or len(labels) != 8:
        errors.append("Must have exactly 8 source labels")
...
def create_usb_camera_source(pipeline, dev_node, width, height, fps, name_suffix, streammux, sink_pad_name):
    ...
    queue.set_property("max-size-buffers", SYNC_SETTINGS.get("queue_max_buffers", 5))
...
def main():
    ...
    streammux.set_property('batch-size', len(OVERLAY_CONFIG.get("source_labels", [])) or 8)
    ...
    print(f"\n *** Launched 8-Camera RTSP Streaming (2 USB + 6 RTSP) ***")
    print(f" *** Output: rtsp://localhost:{RTSP_PORT}{RTSP_MOUNT_POINT_MAIN} ***")
    ...
    print(f" *** Resolution: {FINAL_TILER_WIDTH}x{FINAL_TILER_HEIGHT} @ {USB_CAM_FPS}fps ***\n")
