...
def load_config():
    ...
    settings_path = script_dir / "settings" / "2usb_n_2rtsp_to_file_SETTINGS.json"
    default_path = script_dir / "settings" / "default" / "2usb_n_2rtsp_to_file_SETTINGS_DEFAULT.json"
...
def set_mp4_metadata(mp4mux):
    ...
    taglist.add_value(
        Gst.TagMergeMode.REPLACE,
        Gst.TAG_TITLE,
        f"Quad Camera Recording - {creation_time.strftime('%Y-%m-%d %H:%M:%S')}",
    )
    taglist.add_value(
        Gst.TagMergeMode.REPLACE,
        Gst.TAG_DESCRIPTION,
        "4-camera synchronized recording (2x USB + 2x RTSP)",
    )
    ...
    def readiness_probe(pad, info, u_data):
        ...
        if not sources_ready[source_name]:
            ...
            if all(sources_ready.values()) and not all_sources_ready[0]:
                ...
                print("\n" + "=" * 60)
                print("✓ ALL SOURCES READY - Starting synchronized recording!")
                print("=" * 60 + "\n")
...
def bus_call(bus, message, loop):
    ...
    if t == Gst.MessageType.EOS:
        ...
        if "rtspsrc" in str(debug) or "Could not open resource" in str(err):
            ...
            sys.stderr.write(f"Make sure the RTSP server is running at {RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}\n")
            sys.stderr.write(f"On the remote Jetson, run: python dual_cam_separate_streams.py\n")
            sys.stderr.write("Then verify streams are available:\n")
            sys.stderr.write(f"  rtsp://{RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}{RTSP_CAM1_MOUNT}\n")
            sys.stderr.write(f"  rtsp://{RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}{RTSP_CAM2_MOUNT}\n\n")
...
def create_usb_camera_source(pipeline, dev_node, width, height, fps, name_suffix, streammux, sink_pad_name):
    ...
    # Moderate buffering for USB - balance between sync and responsiveness
    ...
    queue.set_property("max-size-buffers", SYNC_SETTINGS.get("usb_queue_max_buffers", 40))
...
def create_rtsp_source(pipeline, rtsp_location, name_suffix, streammux, sink_pad_name):
    ...
    rtspsrc.set_property("drop-on-latency", True)  # Shed late frames to maintain cadence
    ...
    # Balanced buffering for RTSP (faster startup, good sync)
    queue = Gst.ElementFactory.make("queue", f"queue-{name_suffix}")
    queue.set_property("max-size-time", SYNC_SETTINGS.get("rtsp_queue_max_time_ns", 250000000))
    queue.set_property("max-size-buffers", SYNC_SETTINGS.get("rtsp_queue_max_buffers", 40))
...
def main(args):
    ...
    batched_push_timeout_us = int(SYNC_SETTINGS.get("batched_push_timeout_ms", 35) * 1000)
    ...
    streammux.set_property("max-latency", SYNC_SETTINGS.get("max_latency_ns", 120000000))
    ...
    print("USB queue target time (ns):", SYNC_SETTINGS.get("usb_queue_max_time_ns", 250000000), 
          "buffers:", SYNC_SETTINGS.get("usb_queue_max_buffers", 40))
    print("RTSP queue target time (ns):", SYNC_SETTINGS.get("rtsp_queue_max_time_ns", 250000000), 
          "buffers:", SYNC_SETTINGS.get("rtsp_queue_max_buffers", 40), "leaky downstream")
    print("RTSP src latency (ms):", RTSP_SOURCE_LATENCY_MS, f"protocols: {RTSP_SOURCE_PROTOCOL}", "drop-on-latency:", True)
    print("================================\n")

    print("\n=== Creating Video Sources with Aggressive Buffering ===")

    # Verify RTSP source is reachable
    print(f"\n⚠️  IMPORTANT: Make sure RTSP server is running on {RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}")
    print("   Expected streams:")
    print(f"     - rtsp://{RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}{RTSP_CAM1_MOUNT}")
    print(f"     - rtsp://{RTSP_SOURCE_IP}:{RTSP_SOURCE_PORT}{RTSP_CAM2_MOUNT}")
    print("   Run 'python dual_cam_separate_streams.py' on the remote Jetson first.\n")
    ...
    # Tiler (2x2) - 1080p sources = 3840x2160 output (4K UHD)
    ...
    print("\n *** Launching Quad Camera File Recording with Timestamps ***")
    print(f" *** Output File: {OUTPUT_FILE} ***")
    ...
    print(f" *** Output: {CONFIG['tiler']['width']}x{CONFIG['tiler']['height']} (2x2 tiled 1080p = 4K UHD) ***")
    print(f" *** Encoder: {CONFIG['encoder']['bitrate']/1000000:.0f} Mbps, keyframe every {CONFIG['encoder']['iframeinterval']} frames ***")
    print(f" *** USB Buffer: {SYNC_SETTINGS.get('usb_queue_max_time_ns', 250000000)/1000000:.0f}ms, RTSP Buffer: {SYNC_SETTINGS.get('rtsp_queue_max_time_ns', 250000000)/1000000:.0f}ms, Muxer: {SYNC_SETTINGS.get('batched_push_timeout_ms', 35):.0f}ms ***")
    print(f" *** Expected Sync: ±{SYNC_MONITORING.get('max_latency_variance_ms', 10)}ms @ {USB_CAM_FPS}fps ***\n")
    print("⏳ Waiting for all sources to be ready before starting recording...")
    ...
    if state_return[0] in [Gst.StateChangeReturn.SUCCESS, Gst.StateChangeReturn.NO_PREROLL]:
        ...
        print("Waiting 8 seconds for all sources to connect and stabilize...")
        time.sleep(8)
        print("Sources should be ready now\n")
    else:
        print(f"Warning: Pipeline state change returned: {state_return[0]}\n")
        print("Waiting 8 seconds anyway...")
        time.sleep(8)
